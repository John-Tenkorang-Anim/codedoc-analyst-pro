...
export default function CodeAnalyzer() {
  ...
  const CodeInputSection = () => (
    <div className="flex flex-col gap-4 h-full">
      ...
      <textarea
        ...
        className="w-full p-5 font-mono text-sm code-editor text-gray-100 border-2 border-violet-500/30 rounded-2xl resize-none focus:outline-none focus:ring-2 focus:ring-violet-500 shadow-xl overflow-y-auto flex-1 lg:flex-none"
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          {output && (
            <button
              onClick={handleCopy}
              className="px-3 sm:px-4 py-2 bg-emerald-600/20 hover:bg-emerald-600/30 border-2 border-emerald-500/30 rounded-xl transition-all flex items-center gap-2"
            >
              {copied ? (
                <>
                  <Check className="w-4 h-4 text-emerald-400" />
                  <span className="text-xs sm:text-sm text-emerald-400">Copied!</span>
                </>
              ) : (
                <>
                  <Copy className="w-4 h-4 text-emerald-400" />
                  <span className="text-xs sm:text-sm text-emerald-400 hidden sm:inline">Copy</span>
                </>
              )}
            </button>
          )}
          <button
            onClick={handleReset}
            className="px-3 sm:px-4 py-2 bg-gray-800 hover:bg-gray-700 border-2 border-gray-600 rounded-xl transition-all flex items-center gap-2"
          >
            <RotateCcw className="w-4 h-4 text-gray-400" />
            <span className="text-xs sm:text-sm text-gray-400 hidden sm:inline">Reset</span>
          </button>
        </div>
      </div>

      <div className="flex-1 glass-card p-4 sm:p-6 rounded-2xl overflow-y-auto shadow-xl">
        {loading ? (
          ...
          <ReactMarkdown
            components={{
              ...
              p: ({children}: any) => <p className="mb-4 text-gray-300 leading-relaxed text-sm sm:text-base">{children}</p>,
              h1: ({children}: any) => <h1 className="text-xl sm:text-2xl font-bold mb-4 mt-6 text-violet-300">{children}</h1>,
              h2: ({children}: any) => <h2 className="text-lg sm:text-xl font-bold mb-3 mt-5 text-violet-300">{children}</h2>,
              h3: ({children}: any) => <h3 className="text-base sm:text-lg font-bold mb-2 mt-4 text-violet-300">{children}</h3>,
              ul: ({children}: any) => <ul className="list-disc list-inside mb-4 space-y-1 text-gray-300 text-sm sm:text-base">{children}</ul>,
              ol: ({children}: any) => <ol className="list-decimal list-inside mb-4 space-y-1 text-gray-300 text-sm sm:text-base">{children}</ol>,
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 flex flex-col">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 sm:gap-4">
              <div className="relative">
                <div className="absolute inset-0 bg-gradient-to-br from-violet-500 to-pink-500 rounded-lg sm:rounded-xl blur-sm opacity-75"></div>
                <div className="relative bg-gradient-to-br from-violet-500 to-pink-500 p-2 sm:p-3 rounded-lg sm:rounded-xl">
                  <Code2 className="w-5 h-5 sm:w-7 sm:h-7 text-white" />
                </div>
              </div>
              <div>
                <h1 className="text-lg sm:text-2xl lg:text-3xl font-display font-bold gradient-text">
                  CodeDoc Analyst Pro
                </h1>
                <p className="text-xs sm:text-sm text-gray-400 font-medium mt-0.5 sm:mt-1 hidden sm:block">
      ...
      <div className="border-b border-gray-800 bg-gray-900/50 sticky top-[72px] sm:top-[88px] z-40">
        <div className="max-w-full mx-auto px-3 sm:px-6 lg:px-8">
          <div className="flex gap-1.5 sm:gap-2 overflow-x-auto py-3 sm:py-4 scrollbar-hide">
      ...
      <main className="flex-1 w-full px-3 sm:px-6 lg:px-8 py-6 sm:py-8">
        {/* Desktop: Side by side with resizable panels */}
        <div className="hidden lg:block h-[calc(100vh-18rem)]">
          <PanelGroup direction="horizontal">
            ...
            <Panel defaultSize={45} minSize={25}>
              <div className="h-full pl-3 overflow-y-auto">
                <OutputSection />
              </div>
            </Panel>
          </PanelGroup>
      ...
      <footer className="glass-card border-t border-violet-500/20 mt-auto">
        <div className="max-w-full mx-auto px-3 sm:px-6 lg:px-8 py-3 sm:py-4">
